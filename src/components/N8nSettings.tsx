...
export default function N8nSettings() {
  ...
  if (isLoading) {
    return <div className="text-navy-400 text-sm">Cargando...</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-navy-200">n8n Integration</h3>
        {isConfigured && (
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-ai-green/20 text-ai-green-light">
            Configurado
          </span>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-xs text-navy-300 mb-1">
            Carpeta Google Drive <span className="text-ai-green-light">*</span>
          </label>
          <input
            type="text"
          ...
          <p className="text-[10px] text-navy-500 mt-1">
            ...
            <code className="bg-navy-700 px-1 rounded">Biokool/Catálogos</code>
          </p>
        </div>

        <div>
          <label className="block text-xs text-navy-300 mb-1">Webhook URL</label>
          <input
            type="url"
        ...
        <div>
          <label className="block text-xs text-navy-300 mb-1">Webhook Secret (opcional)</label>
          <input
            type="password"
            value={webhookSecret}
        ...
        <div className="flex gap-2">
          <button
            ...
            className="bg-ai-green hover:bg-ai-green-light text-ai-green-dark text-xs font-bold px-4 py-2 rounded-lg disabled:opacity-50"
          >
          ...
          {isConfigured && (
            <button
              ...
              className="text-red-alert hover:bg-red-dark/20 text-xs font-bold px-4 py-2 rounded-lg"
            >
              Desconectar
            </button>
          )}
        ...
        {saveMutation.isSuccess && (
          <p className="text-xs text-ai-green-light">Configuración guardada</p>
        )}
        {saveMutation.isError && <p className="text-xs text-red-alert">Error al guardar</p>}
      </div>
      ...
      <div className="border-t border-navy-600 pt-3">
        ...
        <p className="text-[11px] text-navy-400 mt-1">
          ...
          <code className="bg-navy-700 px-1 rounded">docs/n8n/document-upload-workflow.json</code>
        </p>
      </div>
    </div>
  );
