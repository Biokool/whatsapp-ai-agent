...
Vas a poner tu agente a correr 24/7 en un servidor. El stack: Hostinger VPS + EasyPanel + Nixpacks (sin Docker). Coste total: 7-13 €/mes.
...
5. (Opcional pero recomendado) Cloudflare Access — ver [05-cloudflare-access.md](05-cloudflare-access.md)
...
2. Pega este comando y dale enter (comando actualizado mayo 2026):
...
En la pestaña **Environment** del app:

```
OPENROUTER_API_KEY=<la key que tienes en tu .env.local local>
...
- Opción A: subdominio de tu dominio. Ej: `panel.tu-empresa.com`
- Opción B: subdominio gratis que da EasyPanel (algo tipo `<id>.easypanel.host`)

Anota el dominio — lo usarás para Cloudflare Access.
...
Antes de meter conversaciones reales, **bloquéalo con Cloudflare Access**. Ver [05-cloudflare-access.md](05-cloudflare-access.md).
